# coding=utf-8
# Copyright 2019 The Allen Institute for Artificial Intelligence.
...
#
#
# NOTE: This file has been forked from
# https://github.com/huggingface/transformers/blob/c56d921dda1ea11870a6cbfb8b64c424543af0e6/transformers/modeling_bert.py
"""PyTorch BERT model. """
...
def load_tf_weights_in_bert(model, config, tf_checkpoint_path):
    ...
    for name, array in zip(names, arrays):
        ...
        pointer.data = torch.from_numpy(array)  # pylint: disable=no-member
    return model
...
def gelu(x):
    ...
    return (
        x
        * 0.5
        * (1.0 + torch.erf(x / math.sqrt(2.0)))  # pylint: disable=no-member
    )
...
def gelu_new(x):
    ...
    return (
        ...
        * (
            1
            + torch.tanh(  # pylint: disable=no-member
                math.sqrt(2 / math.pi)
                * (x + 0.044715 * torch.pow(x, 3))  # pylint: disable=no-member
            )
        )
    )
...
def swish(x):
    return x * torch.sigmoid(x)  # pylint: disable=no-member
...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None, position_ids=None):
        ...
        if position_ids is None:
            position_ids = torch.arange(  # pylint: disable=no-member
                seq_length,
        ...
        if token_type_ids is None:
            token_type_ids = torch.zeros_like(  # pylint: disable=no-member
                input_ids
            )
...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None):
        ...
        attention_scores = torch.matmul(  # pylint: disable=no-member
        ...
        context_layer = torch.matmul(  # pylint: disable=no-member
...
class BertAttention(nn.Module):
    ...
    def prune_heads(self, heads):
        if len(heads) == 0:
            return
        mask = torch.ones(  # pylint: disable=no-member
        ...
        index = torch.arange(len(mask))[  # pylint: disable=no-member
            mask
        ].long()
...
class BertIntermediate(nn.Module):
    def __init__(self, config):
        ...
        if isinstance(config.hidden_act, str) or (
            ...
            and isinstance(
                config.hidden_act, unicode  # pylint: disable=undefined-variable
            )
        ):
...
class BertLMPredictionHead(nn.Module):
    def __init__(self, config):
        ...
        self.bias = nn.Parameter(
            torch.zeros(config.vocab_size)  # pylint: disable=no-member
        )
