...
# https://github.com/huggingface/transformers/blob/c56d921dda1ea11870a6cbfb8b64c424543af0e6/transformers/modeling_roberta.py
...
class RobertaClassificationHead(nn.Module):
    ...
    def forward(self, features, **kwargs):
        ...
        x = self.dropout(x)
        x = self.dense(x)
        x = torch.tanh(x)  # pylint: disable=no-member
        x = self.dropout(x)
        x = self.out_proj(x)
        return x
